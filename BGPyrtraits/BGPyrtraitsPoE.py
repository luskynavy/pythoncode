...
def format_save_filepath(filepath, increment=False):
        ...
        s_name = path.join(folder, filename + "_sm.png")
        ...
        if increment:            
            if path.isfile(s_name):
                ...
                while path.isfile(s_name) and i < 999:
                    s_name = path.join(folder, filename + str(i) + "_sm.png")
                    i+= 1
                filename = filename.replace(" ", "")[0:6] + str(i - 1)
        
        l_name = path.join(folder, filename + "_lg.png")
        m_name = path.join(folder, filename + "_lg.png")
