...
class Application(Frame):
    ...
    def create_labelframes(self, master=None):
        ...
        self.frame_L = LabelFrame(master, text="210x330", height=360, width=230, labelanchor=N)
        self.frame_M = LabelFrame(master, text="169x266 (locked)", height=360, width=230, labelanchor=N)
        self.frame_S = LabelFrame(master, text="54x84 (locked)", height=360, width=230, labelanchor=N)
    ...
    def connectPressed(self, button):
        if self.connect:
            ...
            self.frame_M.configure(text="169x266")
            self.frame_S.configure(text="54x84")
            ...
            self.portrait_M.current_scale = self.portrait_L.current_scale*1.24
            ...
            self.portrait_S.current_scale = self.portrait_L.current_scale*3.93
            ...
            self.frame_M.configure(text="169x266 (locked)")
            self.frame_S.configure(text="54x84 (locked)")
    ...
    def save(self):
        filesnames = format_save_filepath(self.source_image_filepath)
...
def format_save_filepath(filepath):
